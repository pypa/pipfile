...
Generated via ``$ pipenv lock``. See `this example with hashes included <https://raw.githubusercontent.com/pypa/pipfile/master/examples/Pipfile.lock>`_, which can be generated with ``$ pipenv lock --hashes``.
